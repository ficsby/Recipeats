...
class RecipeEditingScreen extends React.Component {
  ...
  renderIngredientsList() {
    return (
      <FlatList
        data={this.state.tempIngredients}
        keyExtractor={(item, index) => index.toString()}
        renderItem={({ item, index }) => (
          <FlatListItem
            parentFlatList={this}
            flatListData={this.state.tempIngredients}
            sectionId={1}
            rowId={index}
            title={item.name}
  ...
  renderInstructionsList() {
    return (
      <FlatList
        data={this.state.tempInstructions}
        keyExtractor={(item, index) => index.toString()}
        renderItem={({ item, index }) => (
          <FlatListItem
            parentFlatList={this}
            flatListData={this.state.tempInstructions}
            sectionId={2}
            rowId={index}
            title={item.step}
            leftIcon={
      ...
      <View>
        ...
        <ScrollView style={styles.recipeContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={this.onCancelEditChanges}
          >
